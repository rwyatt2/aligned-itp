...
interface AlignedLogoProps {
  className?: string
  size?: number | string
  animated?: boolean | 'scroll' | 'breathing' | 'draw' | 'converge' | 'outline' | 'reveal'
  color?: string
}
...
export default function AlignedLogo({
  ...
  // ── Reveal (stroke-draw → fill) ────────────────────────────────────────────
  // Industry-standard logo animation: the outline draws itself via pathLength,
  // then the fill fades in while the stroke fades out. Uses the single compound
  // path — no decomposition, no clipping, no coordinate mismatches.
  if (animated === 'reveal') {
        ...
        <svg viewBox="0 0 1121 974" width={size} height={size} className="w-full h-auto max-w-full">
          <motion.path
            ...
            onAnimationComplete={(def) => {
              // Only trigger done when fillOpacity animation completes
              if (typeof def === 'string' && def === 'fillOpacity') return
              setDone(true)
            }}
          />
        </svg>
  ...
  return (
    <div ref={containerRef} className={className}>
      <motion.svg
        ...
        height={size}
        className="w-full h-auto max-w-full"
        style={{ opacity: overallOpacity, scale }}
      >
        <path d={pathD} fill={color} />
      </motion.svg>
    </div>
